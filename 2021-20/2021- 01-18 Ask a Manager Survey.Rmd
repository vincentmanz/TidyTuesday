...
p_median <- tab  %>% ggplot(aes(x = Experience, group = 1)) +
  ...
  geom_curve(data = arrow[1,], aes(x = x, xend = xend, y = y, yend = yend), 
             arrow = arrow(length = unit(0.17, "inch")),  size = 0.5, color = "#166FCC", curvature = -0.35) +
  geom_curve(data = arrow[2,], aes(x = x, xend = xend, y = y, yend = yend), 
             arrow = arrow(length = unit(0.17, "inch")),  size = 0.5, color = "#FF4040", curvature = -0.35) +
  geom_text(data = annotation[1,], label = "Men", size = 3,
              color = "#166FCC", family = "Poppins", fontface = "bold", lineheight = 0.8) +
...
ggsave("2021-01-18-Ask-a-Manager-Survey.pdf", width = 14, height = 5.6, device = pdf)
```
